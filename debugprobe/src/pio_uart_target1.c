...
#define TARGET1_BAUD          9600

static PIO s_pio = TARGET1_UART_PIO;
static uint s_sm_tx = TARGET1_UART_TX_SM;
static uint s_sm_rx = TARGET1_UART_RX_SM;
#include "tusb.h"
...
void target1_pio_uart_usb_task(uint8_t cdc_itf) {  
    ...
    if (tud_cdc_n_connected(cdc_itf)) {
        while (tud_cdc_n_write_available(cdc_itf)) {
            ...
            if (!target1_pio_uart_read_byte(&ch)) {
                break;
            }
...
void target1_pio_uart_write(const uint8_t *data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        target1_pio_uart_write_byte(data[i]);
    }
}
