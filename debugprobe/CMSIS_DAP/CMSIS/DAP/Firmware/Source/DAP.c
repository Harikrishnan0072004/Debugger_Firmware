...
DAP_Data_t DAP_Data[PROBE_DAP_COUNT];           // DAP Data
volatile uint8_t    DAP_TransferAbort[PROBE_DAP_COUNT];  // Transfer Abort Flag
...
void Delayms(uint32_t probeId, uint32_t delay) {
...
static uint32_t DAP_Delay(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t delay;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_HostStatus(uint32_t probeId, const uint8_t *request, uint8_t *response) {
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_Connect(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t port;
...
#if (DAP_SWD != 0)
    case DAP_PORT_SWD:
      DAP_Data[probeId].debug_port = DAP_PORT_SWD;
      PORT_SWD_SETUP(probeId);
      break;
#endif
#if (DAP_JTAG != 0)
    case DAP_PORT_JTAG:
      DAP_Data[probeId].debug_port = DAP_PORT_JTAG;
      PORT_JTAG_SETUP(probeId);
      break;
#endif
    default:
...
static uint32_t DAP_Disconnect(uint32_t probeId, uint8_t *response) {

  DAP_Data[probeId].debug_port = DAP_PORT_DISABLED;
  PORT_OFF(probeId);
...
//   response: pointer to response data
//   return:   number of bytes in response
static uint32_t DAP_ResetTarget(uint32_t probeId, uint8_t *response) {

  *(response+1) = RESET_TARGET(probeId);
  *(response+0) = DAP_OK;
  return (2U);
}
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_SWJ_Pins(uint32_t probeId, const uint8_t *request, uint8_t *response) {
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_SWJ_Clock(uint32_t probeId, const uint8_t *request, uint8_t *response) {
#if ((DAP_SWD != 0) || (DAP_JTAG != 0))
  uint32_t clock;
  uint32_t delay;
  ...
  if (clock >= MAX_SWJ_CLOCK(DELAY_FAST_CYCLES)) {
    DAP_Data[probeId].fast_clock  = 1U;
    DAP_Data[probeId].clock_delay = 1U;
  } else {
    DAP_Data[probeId].fast_clock  = 0U;
    ...
    DAP_Data[probeId].clock_delay = delay;
  }
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_SWJ_Sequence(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t count;
...
#if ((DAP_SWD != 0) || (DAP_JTAG != 0))
  SWJ_Sequence(probeId, count, request);
  *response = DAP_OK;
#else
  *response = DAP_ERROR;
#endif
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_SWD_Configure(uint32_t probeId, const uint8_t *request, uint8_t *response) {
#if (DAP_SWD != 0)
  uint8_t value;

  value = *request;
  DAP_Data[probeId].swd_conf.turnaround = (value & 0x03U) + 1U;
  DAP_Data[probeId].swd_conf.data_phase = (value & 0x04U) ? 1U : 0U;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_SWD_Sequence(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  ...
  *response++ = DAP_ERROR;
  ...
  while (sequence_count--) {
    ...
    if ((sequence_info & SWD_SEQUENCE_DIN) != 0U) {
      PIN_SWDIO_OUT_DISABLE(probeId);
    } else {
      PIN_SWDIO_OUT_ENABLE(probeId);
    }
    SWD_Sequence(probeId, sequence_info, request, response);
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_JTAG_Sequence(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t sequence_info;
  uint32_t sequence_count;
  uint32_t request_count;
  uint32_t response_count;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_JTAG_Configure(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t count;
#if (DAP_JTAG != 0)
  uint32_t length;
  uint32_t bits;
  ...
  count = *request++;
  DAP_Data[probeId].jtag_dev.count = (uint8_t)count;

  bits = 0U;
  for (n = 0U; n < count; n++) {
    length = *request++;
    DAP_Data[probeId].jtag_dev.ir_length[n] =  (uint8_t)length;
    DAP_Data[probeId].jtag_dev.ir_before[n] = (uint16_t)bits;
    bits += length;
  }
  for (n = 0U; n < count; n++) {
    bits -= DAP_Data[probeId].jtag_dev.ir_length[n];
    DAP_Data[probeId].jtag_dev.ir_after[n] = (uint16_t)bits;
  }
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_JTAG_IDCode(uint32_t probeId, const uint8_t *request, uint8_t *response) {
#if (DAP_JTAG != 0)
  uint32_t data;

  if (DAP_Data[probeId].debug_port != DAP_PORT_JTAG) {
    goto id_error;
  }

  // Device index (JTAP TAP)
  DAP_Data[probeId].jtag_dev.index = *request;
  if (DAP_Data[probeId].jtag_dev.index >= DAP_Data[probeId].jtag_dev.count) {
    goto id_error;
  }
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_TransferConfigure(uint32_t probeId, const uint8_t *request, uint8_t *response) {

  DAP_Data[probeId].transfer.idle_cycles =            *(request+0);
  DAP_Data[probeId].transfer.retry_count = (uint16_t) *(request+1) |
                                  (uint16_t)(*(request+2) << 8);
  DAP_Data[probeId].transfer.match_retry = (uint16_t) *(request+3) | 
...
//             number of bytes in request (upper 16 bits)
#if (DAP_SWD != 0)
static uint32_t DAP_SWD_Transfer(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  const
  ...
  for (; request_count != 0U; request_count--) {
    ...
    if ((request_value & DAP_TRANSFER_RnW) != 0U) {
      ...
      if (post_read) {
        ...
        retry = DAP_Data[probeId].transfer.retry_count;
        if ((request_value & (DAP_TRANSFER_APnDP | DAP_TRANSFER_MATCH_VALUE)) == DAP_TRANSFER_APnDP) {
          ...
          do {
            response_value = SWD_Transfer(probeId, request_value, &data);
          } while ((response_value == DAP_TRANSFER_WAIT) && retry-- && !DAP_TransferAbort[probeId]);
        } else {
          // Read previous AP data
          do {
            response_value = SWD_Transfer(probeId, DP_RDBUFF | DAP_TRANSFER_RnW, &data);
          } while ((response_value == DAP_TRANSFER_WAIT) && retry-- && !DAP_TransferAbort[probeId]);
          post_read = 0U;
        }
        ...
        *response++ = (uint8_t)(data >> 24);
        ...
        if (post_read) {
          ...
          if ((request_value & DAP_TRANSFER_TIMESTAMP) != 0U) {
            timestamp = DAP_Data[probeId].timestamp;
      ...
      if ((request_value & DAP_TRANSFER_MATCH_VALUE) != 0U) {
        ...
        match_value = (uint32_t)(*(request+0) <<  0) |
                      ...
                      (uint32_t)(*(request+3) << 24);
        request += 4;
        match_retry = DAP_Data[probeId].transfer.match_retry;
        if ((request_value & DAP_TRANSFER_APnDP) != 0U) {
          // Post AP read
          retry = DAP_Data[probeId].transfer.retry_count;
          do {
            response_value = SWD_Transfer(probeId, request_value, NULL);
          } while ((response_value == DAP_TRANSFER_WAIT) && retry-- && !DAP_TransferAbort[probeId]);
          if (response_value != DAP_TRANSFER_OK) {
            break;
          }
        ...
        } while (((data & DAP_Data[probeId].transfer.match_mask) != match_value) && match_retry-- && !DAP_TransferAbort[probeId]);
        if ((data & DAP_Data[probeId].transfer.match_mask) != match_value) {
      ...
      if ((request_value & DAP_TRANSFER_MATCH_MASK) != 0U) {
        // Write match mask
        DAP_Data[probeId].transfer.match_mask = data;
  ...
  if (response_value == DAP_TRANSFER_OK) {
    if (post_read) {
      ...
      *response++ = (uint8_t)(data >> 24);
      ...
      retry = DAP_Data[probeId].transfer.retry_count;
      do {
        response_value = SWD_Transfer(probeId, DP_RDBUFF | DAP_TRANSFER_RnW, NULL);
      } while ((response_value == DAP_TRANSFER_WAIT) && retry-- && !DAP_TransferAbort[probeId]);
    }
  }
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_Dummy_Transfer(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  const
  uint8_t  *request_head;
  uint32_t  request_count;
  uint32_t  request_value;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_Transfer(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t num;

  switch (DAP_Data[probeId].debug_port) {
#if (DAP_SWD != 0)
    case DAP_PORT_SWD:
      num = DAP_SWD_Transfer(probeId, request, response);
      break;
#endif
#if (DAP_JTAG != 0)
    case DAP_PORT_JTAG:
      num = DAP_JTAG_Transfer(probeId, request, response);
      break;
#endif
    default:
      num = DAP_Dummy_Transfer(probeId, request, response);
      break;
  }
...
//   return:   number of bytes in response
#if (DAP_SWD != 0)
static uint32_t DAP_SWD_TransferBlock(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t  request_count;
  uint32_t  request_value;
  uint32_t  response_count;
  uint32_t  response_value;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_TransferBlock(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t num;

  switch (DAP_Data[probeId].debug_port) {
#if (DAP_SWD != 0)
    case DAP_PORT_SWD:
      num = DAP_SWD_TransferBlock (probeId, request, response);
      break;
#endif
#if (DAP_JTAG != 0)
    case DAP_PORT_JTAG:
...
//   return:   number of bytes in response
#if (DAP_SWD != 0)
static uint32_t DAP_SWD_WriteAbort(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t data;
  ...
  SWD_Transfer(probeId, DP_ABORT, &data);
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static uint32_t DAP_WriteAbort(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t num;

  switch (DAP_Data[probeId].debug_port) {
#if (DAP_SWD != 0)
    case DAP_PORT_SWD:
      num = DAP_SWD_WriteAbort (probeId, request, response);
      break;
#endif
#if (DAP_JTAG != 0)
    case DAP_PORT_JTAG:
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
__WEAK uint32_t DAP_ProcessVendorCommand(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  (void)request;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
uint32_t DAP_ProcessCommand(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t num;

  if ((*request >= ID_DAP_Vendor0) && (*request <= ID_DAP_Vendor31)) {
    return DAP_ProcessVendorCommand(probeId, request, response);
  }
  ...
  *response++ = *request;
  ...
  switch (*request++) {
    ...
    case ID_DAP_HostStatus:
      num = DAP_HostStatus(probeId, request, response);
      break;

    case ID_DAP_Connect:
      num = DAP_Connect(probeId, request, response);
      break;
    case ID_DAP_Disconnect:
      num = DAP_Disconnect(probeId, response);
      break;

    case ID_DAP_Delay:
      num = DAP_Delay(probeId, request, response);
      break;

    case ID_DAP_ResetTarget:
      num = DAP_ResetTarget(probeId, response);
      break;

    case ID_DAP_SWJ_Pins:
      num = DAP_SWJ_Pins(probeId, request, response);
      break;
    case ID_DAP_SWJ_Clock:
      num = DAP_SWJ_Clock(probeId, request, response);
      break;
    case ID_DAP_SWJ_Sequence:
      num = DAP_SWJ_Sequence(probeId, request, response);
      break;

    case ID_DAP_SWD_Configure:
      num = DAP_SWD_Configure(probeId, request, response);
      break;
    case ID_DAP_SWD_Sequence:
      num = DAP_SWD_Sequence(probeId, request, response);
      break;

    case ID_DAP_JTAG_Sequence:
      num = DAP_JTAG_Sequence(probeId, request, response);
      break;
    case ID_DAP_JTAG_Configure:
      num = DAP_JTAG_Configure(probeId, request, response);
      break;
    case ID_DAP_JTAG_IDCODE:
      num = DAP_JTAG_IDCode(probeId, request, response);
      break;

    case ID_DAP_TransferConfigure:
      num = DAP_TransferConfigure(probeId, request, response);
      break;
    case ID_DAP_Transfer:
      num = DAP_Transfer(probeId, request, response);
      break;
    case ID_DAP_TransferBlock:
      num = DAP_TransferBlock(probeId, request, response);
      break;

    case ID_DAP_WriteABORT:
      num = DAP_WriteAbort(probeId, request, response);
      break;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
uint32_t DAP_ExecuteCommand(uint32_t probeId, const uint8_t *request, uint8_t *response) {
  uint32_t cnt, num, n;
  ...
  if (*request == ID_DAP_ExecuteCommands) {
    ...
    *response++ = (uint8_t)cnt;
    num = (2U << 16) | 2U;
    while (cnt--) {
      n = DAP_ProcessCommand(probeId, request, response);
      num += n;
  ...
  return DAP_ProcessCommand(probeId, request, response);
}
...
void DAP_Setup(uint32_t probeId) {
  ...
  // Default settings
  DAP_Data[probeId].debug_port  = 0U;
  DAP_Data[probeId].fast_clock  = 0U;
  DAP_Data[probeId].clock_delay = CLOCK_DELAY(DAP_DEFAULT_SWJ_CLOCK);
  DAP_Data[probeId].transfer.idle_cycles = 0U;
  DAP_Data[probeId].transfer.retry_count = 100U;
  DAP_Data[probeId].transfer.match_retry = 0U;
  DAP_Data[probeId].transfer.match_mask  = 0x00000000U;
#if (DAP_SWD != 0)
  DAP_Data[probeId].swd_conf.turnaround  = 1U;
  DAP_Data[probeId].swd_conf.data_phase  = 0U;
#endif
#if (DAP_JTAG != 0)
  DAP_Data[probeId].jtag_dev.count = 0U;
#endif

  DAP_SETUP(probeId);  // Device specific setup
}
